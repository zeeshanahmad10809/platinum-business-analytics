from flask import Flask, request, make_response, render_template, url_for, session, redirect, jsonify, send_from_directory, current_app
...
app.config['MAIL_PASSWORD'] = "charra'sbusiness"
...
def get_embeddings(sentences_tokens):
    ...
    for sentence_token in sentences_tokens:
        sentences_embed.append(torch.tensor(tokenizer.encode("[CLS]"+' '.join(sentence_token)+"[SEP]")))
...
def checkAmazonURLExist(url):
    ...
    while True:
        if driver.find_element_by_xpath('//title').get_attribute('innerText') == 'Robot Check':
            ...
            title = driver.find_element_by_xpath("//title").get_attribute("innerText").strip()
...
def projects_endpoint():
    ...
    try:
        data = list(mongo.db.ProjectData.find({"user_email": session['user']['user_email']},
        ...
        if len(data['projects']) > 0:
            for project in data['projects']:
                project['update_count'] = count_updates(user_id, ObjectId(project['_id']))
            data = data['projects']
        else:
            data = []
...
@app.route('/dashboard/project_name=<project_name>/project_category=<project_category>/')
def dashboard_endpoint(project_name, project_category):
    ...
    return render_template('full_dashboard.html', session=session, reviews_list=data, project_name=project_name)
...
def create_project_endpoint():
    ...
    if 'is_duplicate_project' in session.keys():
        ...
        return render_template('create_project.html', session=session, is_duplicate=True, invalid_reviews_url=False, invalid_search_query=False)
    if 'invalid_reviews_url' in session.keys():
        session.pop('invalid_reviews_url')
        return render_template('create_project.html', session=session, is_duplicate=False, invalid_reviews_url=True, invalid_search_query=False)
    if 'invalid_search_query' in session.keys():
        session.pop('invalid_search_query')
        return render_template('create_project.html', session=session, is_duplicate=False, invalid_reviews_url=False, invalid_search_query=True)


    return render_template('create_project.html', session=session, is_duplicate=False, invalid_reviews_url=False, invalid_search_query=False)
...
def login_endpoint():
    ...
    if 'invalid_cred' in session.keys():
        if session['invalid_cred'] == True:
            ...
            return render_template('login.html', invalid_cred=True, reset_link_sent=False, is_password_changed=False)
    if 'reset_link_sent' in session.keys():
        if session['reset_link_sent'] == True:
            session.pop('reset_link_sent')
            return render_template('login.html', invalid_cred=False, reset_link_sent=True, is_password_changed=False)
    if 'is_password_changed' in session.keys():
        if session['is_password_changed'] == True:
            session.pop('is_password_changed')
            return render_template('login.html', invalid_cred=False, reset_link_sent=False, is_password_changed=True)

    return render_template('login.html', invalid_cred=False, reset_link_sent=False, is_password_changed=False)
...
def register_endpoint():
    ...
    if 'invalid_provided_email' in session.keys():
        if session['invalid_provided_email'] == True:
            ...
            return render_template('register.html', already_account=False, invalid_provided_email=True, invalid_reg_password=False, invalid_reg_repeat_password=False)
    if 'already_account' in session.keys():
        if session['already_account'] == True:
            session.pop('already_account')
            return render_template('register.html', already_account=True, invalid_provided_email=False, invalid_reg_password=False, invalid_reg_repeat_password=False)
    if 'invalid_reg_repeat_password' in session.keys():
        if session['invalid_reg_repeat_password'] == True:
            ...
            return render_template('register.html', already_account=False, invalid_provided_email=False, invalid_reg_password=False, invalid_reg_repeat_password=True)
    if 'invalid_reg_password' in session.keys():
        if session['invalid_reg_password'] == True:
            session.pop('invalid_reg_password')
            return render_template('register.html', already_account=False, invalid_provided_email=False, invalid_reg_password=True, invalid_reg_repeat_password=False)
    return render_template('register.html', already_account=False, invalid_provided_email=False, invalid_reg_password=False, invalid_reg_repeat_password=False)
...
def register_user():
    ...
    try:
        ...
        if not user:
            ...
            pwd_reg_result = re.match('^(?=\S{6,20}$)(?=.*?\d)(?=.*?[a-z])(?=.*?[A-Z])(?=.*?[^A-Za-z\s0-9])', password)
            ...
            mongo.db.ProjectData.insert_one({"user_email": request.form.get("email"),
                                             "user_password": generate_password_hash(
                                                 request.form.get("password"), method='sha256'),
                                             "subscribed": True}) #Added subscribed...
            user = mongo.db.ProjectData.find_one({"user_email": request.form.get("email")})
            session['user'] = {"user_email": request.form.get("email"), "_id": str(user["_id"]).split("'")[0]}
            session.permanent = True
            message = Message('Account Created!', sender=app.config.get('MAIL_USERNAME'),
                              recipients=[address])
            message.html = "<h5>" + "Welcome, Your account has been created successfully!" + "</h5>"
            try:
                mail.send(message)
            except:
                pass
...
def user_login():
    ...
    if check_password_hash(user['user_password'], password):
        session['user'] = {"user_email": email, "_id": str(user["_id"]).split("'")[0], "pass": password}
        session.permanent = True
        ...
        if project_category == "Product":
            ...
            if re.match(
                ...
                if checkAmazonURLExist(product_url) == True:
                    ...
                    product_url = 'https://www.amazon.com/2019-Stereo-Headphones-Samsung-Galaxy/product-reviews/' + product_id + '/ref=cm_cr_arp_d_viewopt_srt?ie=UTF8&reviewerType=all_reviews&sortBy=recent&pageNumber=1'
            ...
            project_count = mongo.db.ProjectData.find({"user_email": session["user"]["user_email"],
                                                       "projects": {"$elemMatch": {
                                                           "project_category": project_category,
                                                           "project_name": project_name
                                                       }}}).count()
            if project_count == 0:
                project_id = ObjectId()
                mongo.db.ProjectData.update({"user_email": session["user"]["user_email"]},
                                            {"$push": {"projects": {
                                                "_id": project_id,
                                                "project_name": project_name,
                                                "project_category": project_category,
                                                ...
                                                "timestamp": datetime.datetime.strptime(str(datetime.datetime.utcnow().date()), "%Y-%m-%d")
                                            }}})

                logging.warning("Before start of amazon scraping id="+str(id(project_id)))
                def scrap_amazon(url, user_id, project_id, timestamp):
                    os.chdir('Amazon')

                    subprocess.check_output(['scrapy', 'crawl', 'amazon_reviews', '-a', 'url=' + url,
                                             '-a', 'user_id=' + user_id,
                ...
                thread_runner = threading.Thread(target=scrap_amazon, args=(str(product_url), str(session["user"]["_id"]), str(project_id), str(None),))
                thread_runner.start()
                time.sleep(60)
                #scrap_amazon()
                return redirect(url_for('panel', project_name=project_name,
                                        project_category=project_category))
            else:
            ...
            if re.match('^https\:\/\/www\.rottentomatoes\.com\/m\/[a-zA-Z0-9_]*\/reviews\?type\=user$',
                        product_url) is not None or re.match(
                '^https\:\/\/www\.rottentomatoes\.com\/tv\/[a-zA-Z0-9_]*\/s[0-9]*\/reviews\?type\=user$',
                product_url) is not None:
                if checkRottenTomatoesURLExist(product_url) == True:  # checkRottenTomatoesURLExist validate if link exists or 404..
                    pass
                else:
                    session['invalid_reviews_url'] = True
                    return redirect(url_for('create_project_endpoint'))
            ...
            if project_count == 0:
                ...
                logging.warning("Before start of rotten tomatoes scraping id=" + str(id(project_id)))

                def scrap_rotten_tomatoes(product_url, user_id, project_id, timestamp):
                    os.chdir('Rotten_Tomatoes')
                    subprocess.check_output(['scrapy', 'crawl', 'rt_reviews', '-a', 'url=' + product_url,
                                             '-a', 'user_id=' + user_id,
                                             '-a', 'project_id=' + project_id,
                                             '-a', 'timestamp=' + timestamp])
                ...
                thread_runner = threading.Thread(target=scrap_rotten_tomatoes, args=(str(product_url), str(session["user"]["_id"]), str(project_id), str(None),))
                thread_runner.start()
                time.sleep(60)
            ...
            if project_count == 0:
                ...
                logging.warning("Before start of google scraping id=" + str(id(project_id)))

                def scrap_google(query, user_id, project_id, timestamp):
                    os.chdir('Google')
                    subprocess.check_output(['scrapy', 'crawl', 'google_results', '-a', 'query=' + query,
                                             '-a', 'user_id=' + user_id,
                                             '-a', 'project_id=' + project_id,
                                             '-a', 'timestamp=' + timestamp])
                ...
                thread_runner = threading.Thread(target=scrap_google, args=(str(product_url), str(session["user"]["_id"]), str(project_id), str(None),))
                thread_runner.start()
                time.sleep(60)
                #scrap_google()
                return redirect(url_for('panel', project_name=project_name,
...
@app.route('/delete_project/project_name=<project_name>/project_category=<project_category>/')
def delete_project(project_name, project_category):
    result = mongo.db.ProjectData.update({"user_email": session['user']['user_email']},
                                         {"$pull": {"projects": {
                                             "project_name": project_name,
                                             "project_category": project_category
                                         }}})
    ...
    try:
        ...
        if 'forget_re_password_invalid' in session.keys():
            if session['forget_re_password_invalid'] == True:
                ...
                return render_template('reset_password.html', token=token, reset_email=email, forget_re_password_invalid=True, forget_password_regex_not_match=False)
        ...
        if 'forget_password_regex_not_match' in session.keys():
            if session['forget_password_regex_not_match'] == True:
                ...
                return render_template('reset_password.html', token=token, reset_email=email, forget_re_password_invalid=False, forget_password_regex_not_match=True)

        return render_template('reset_password.html', token=token, reset_email=email, forget_re_password_invalid=False, forget_password_regex_not_match=False)
...
def reset_password():
    ...
    try:
        ...
        email = url_serializer.loads(token, salt='thisisemailsalt', max_age=3600)
        mongo.db.ProjectData.update({"user_email": email},
...
def getCollectionSize(project_name, project_category, sentiment_filter, date_start, date_end):
        ...
        if project_category == 'General':
            data = mongo.db.ProjectData.aggregate([
                {"$match": {
                    ...
                    "projects.project_name": project_name,
                    "projects.project_category": project_category,
                    "projects.data.publish_date": {'$lt': date_end, "$gte": date_start},
            ...
            data = mongo.db.ProjectData.aggregate([
                {"$match": {
                    ...
                    "projects.project_name": project_name,
                    "projects.project_category": project_category,
                    "projects.data.review_date": {'$lt': date_end, "$gte": date_start},
...
def getTimeSeriesData(project_name, project_category, sentiment_filter, date_start, date_end):
    if sentiment_filter == None and date_start == None and date_end == None:
        if project_category == 'General':
            data = mongo.db.ProjectData.aggregate([
                {"$match": {
...
def getBarPieData(project_name, project_category, sentiment_filter, date_start, date_end):
    if sentiment_filter == None and date_start == None and date_end == None:
        if project_category == 'General':
            data = mongo.db.ProjectData.aggregate([
                {"$match": {
                ...
                {"$match": {
                    "projects.project_name": project_name,
                    "projects.project_category": project_category,
                    "projects.data.mention_sentiment": {"$in": ['0', '1', '2', '3', '4']}
                }},
            ...
            data = mongo.db.ProjectData.aggregate([
                ...
                {"$match": {
                    "projects.project_name": project_name,
                    "projects.project_category": project_category,
                    "projects.data.review_sentiment": {"$in": ['0', '1', '2', '3', '4']}
                }},
...
def get_project_data(project_name, project_category, page_no, page_start, page_size, sentiment_filter, date_start, date_end):
    if project_category == "General":
        if sentiment_filter == None:
            if date_start == None and date_end == None:
                data = mongo.db.ProjectData.aggregate([
                    ...
                    {"$group": {
                        "_id": {"user_email": "$user_email", "project_name": "$projects.project_name",
                                "project_category": "$projects.project_category"},
                        "data": {"$push": {"mention_category": "$projects.data.mention_category",
                    ...
                    {"$group": {
                        "_id": "$_id.user_email",
                        "projects": {"$push": {
                            "data": {"data": {"$slice": ["$data", page_start, page_size]}}
                        }}
                    }}
                ])
                data = list(data)
                if len(data) > 0:
                    ...
                    if page_no == 1:
                        data['size'] = getCollectionSize(project_name, 'General', None, None, None)
                        data['time_series_data'] = getTimeSeriesData(project_name, 'General', None, None, None)
                        data['bar_pie_data'] = getBarPieData(project_name, 'General', None, None, None)
                ...
                if len(data) > 0:
                    data = data[0]['projects'][0]['data']
                    if page_no == 1:
                        data['size'] = getCollectionSize(project_name, 'General', None, date_start, date_end)
                        data['time_series_data'] = getTimeSeriesData(project_name, 'General', None, date_start,
                                                                     date_end)
                        data['bar_pie_data'] = getBarPieData(project_name, 'General', None, date_start, date_end)
            ...
            if date_start == None and date_end == None:
                ...
                if len(data) > 0:
                    data = data[0]['projects'][0]['data']
                    if page_no == 1:
                        data['size'] = getCollectionSize(project_name, 'General', sentiment_filter, None, None)
                        data['time_series_data'] = getTimeSeriesData(project_name, 'General', sentiment_filter, None,
                                                                     None)
                        data['bar_pie_data'] = getBarPieData(project_name, 'General', sentiment_filter, None, None)
                ...
                if len(data) > 0:
                    data = data[0]['projects'][0]['data']
                    if page_no == 1:
                        data['size'] = getCollectionSize(project_name, 'General', sentiment_filter, date_start,
                                                         date_end)
                        data['time_series_data'] = getTimeSeriesData(project_name, 'General', sentiment_filter,
                                                                     date_start, date_end)
                        data['bar_pie_data'] = getBarPieData(project_name, 'General', sentiment_filter, date_start,
                                                             date_end)
        ...
        if sentiment_filter == None:
            ...
            if date_start == None and date_end == None:
                data = mongo.db.ProjectData.aggregate([
                    ...
                    {"$group": {
                        "_id": {"user_email": "$user_email", "project_name": "$projects.project_name",
                                "project_category": "$projects.project_category"},
                        "data": {"$push": {"user_picture": "$projects.data.user_picture",
                ...
                if len(data) > 0:
                    data = data[0]['projects'][0]['data']
                    if page_no == 1:
                        data['size'] = getCollectionSize(project_name, 'Product', None, None, None)
                        data['time_series_data'] = getTimeSeriesData(project_name, 'Product', None, None, None)
                        data['bar_pie_data'] = getBarPieData(project_name, 'Product', None, None, None)
                ...
                if len(data) > 0:
                    data = data[0]['projects'][0]['data']
                    if page_no == 1:
                        data['size'] = getCollectionSize(project_name, 'Product', None, date_start, date_end)
                        data['time_series_data'] = getTimeSeriesData(project_name, 'Product', None, date_start,
                                                                     date_end)
                        data['bar_pie_data'] = getBarPieData(project_name, 'Product', None, date_start, date_end)
            ...
            if date_start == None and date_end == None:
                ...
                if len(data) > 0:
                    data = data[0]['projects'][0]['data']
                    if page_no == 1:
                        data['size'] = getCollectionSize(project_name, 'Product', sentiment_filter, None, None)
                        data['time_series_data'] = getTimeSeriesData(project_name, 'Product', sentiment_filter, None,
                                                                     None)
                        data['bar_pie_data'] = getBarPieData(project_name, 'Product', sentiment_filter, None, None)
                ...
                if len(data) > 0:
                    data = data[0]['projects'][0]['data']
                    if page_no == 1:
                        data['size'] = getCollectionSize(project_name, 'Product', sentiment_filter, date_start,
                                                         date_end)
                        data['time_series_data'] = getTimeSeriesData(project_name, 'Product', sentiment_filter,
                                                                     date_start, date_end)
                        data['bar_pie_data'] = getBarPieData(project_name, 'Product', sentiment_filter, date_start,
                                                             date_end)
        ...
        if sentiment_filter == None:
            ...
            if date_start == None and date_end == None:
                ...
                if len(data) > 0:
                    data = data[0]['projects'][0]['data']
                    if page_no == 1:
                        data['size'] = getCollectionSize(project_name, 'Movie', None, None, None)
                        data['time_series_data'] = getTimeSeriesData(project_name, 'Movie', None, None, None)
                        data['bar_pie_data'] = getBarPieData(project_name, 'Movie', None, None, None)
                ...
                if len(data) > 0:
                    data = data[0]['projects'][0]['data']
                    if page_no == 1:
                        data['size'] = getCollectionSize(project_name, 'Movie', None, date_start, date_end)
                        data['time_series_data'] = getTimeSeriesData(project_name, 'Movie', None, date_start, date_end)
                        data['bar_pie_data'] = getBarPieData(project_name, 'Movie', None, date_start, date_end)
            ...
            if date_start == None and date_end == None:
                ...
                if len(data) > 0:
                    data = data[0]['projects'][0]['data']
                    if page_no == 1:
                        data['size'] = getCollectionSize(project_name, 'Movie', sentiment_filter, None, None)
                        data['time_series_data'] = getTimeSeriesData(project_name, 'Movie', sentiment_filter, None,
                                                                     None)
                        data['bar_pie_data'] = getBarPieData(project_name, 'Movie', sentiment_filter, None, None)
                ...
                if len(data) > 0:
                    data = data[0]['projects'][0]['data']
                    if page_no == 1:
                        data['size'] = getCollectionSize(project_name, 'Movie', sentiment_filter, date_start, date_end)
                        data['time_series_data'] = getTimeSeriesData(project_name, 'Movie', sentiment_filter,
                                                                     date_start, date_end)
                        data['bar_pie_data'] = getBarPieData(project_name, 'Movie', sentiment_filter, date_start,
                                                             date_end)
...
def get_page_general(page_no):
    ...
    if sentiment_filter is None and date_start is None and date_end is None and page_no == 1:
        try:
            mongo.db.ProjectData.update({"user_email": session['user']['user_email'], "projects.project_name": project_name,
                                        "projects.project_category": "General"},
    ...
    return get_project_data(project_name, "General", page_no, page_start, page_size, sentiment_filter, date_start, date_end)
...
def get_page_product(page_no):
    ...
    if sentiment_filter is None and date_start is None and date_end is None and page_no == 1:
        try:
            mongo.db.ProjectData.update(
                {"user_email": session['user']['user_email'], "projects.project_name": project_name,
                 "projects.project_category": "Product"},
                {"$set": {"projects.$.data.$[].seen": True
                          }})
        except:
    ...
    return get_project_data(project_name, "Product", page_no, page_start, page_size, sentiment_filter, date_start, date_end)
...
def get_page_movie(page_no):
    ...
    return get_project_data(project_name, "Movie", page_no, page_start, page_size, sentiment_filter, date_start, date_end)
...
def renderTimeSeriesGraph(project_name, project_category, time_series_data):
    ...
    fig.update_layout(
        ...
        width=850,
        height=400)
    svg_name = session['user']['user_email']+project_name+project_category+"time-series-graph.svg"
...
def renderBarGraph(project_name, project_category, bar_pie_data):
    color_map = {"Negative": '#ff0000', "Neutral": '#c0c0c0', "Partially Negative": '#c41e3a',
                  "Partially Positive": '#4f7942', "Positive": '#228b22'}
    label_map = {"negative": "Negative", "neutral": "Neutral", "partially_negative": "Partially Negative",
                 "partially_positive": "Partially Positive", "positive": "Positive"}
    labels = [label_map[dv] for dv in bar_pie_data]
    values = [bar_pie_data[dv] for dv in bar_pie_data]
    fig = go.Figure(data=[go.Bar(
        x=labels,
        y=values,
        marker_color=[color_map[label] for label in labels]  # marker color can be a single color value or an iterable
    )])
    ...
    fig.update_layout(
        ...
        width=800,
        height=500)
    svg_name = session['user']['user_email'] + project_name + project_category + "bar-graph.svg"
    svg_path = 'static/graph/' + svg_name
    if 'to_delete' not in session.keys():
        session['to_delete'] = []
    session['to_delete'].append(svg_path)
...
def renderPieGraph(project_name, project_category, bar_pie_data):
    ...
    svg_name = session['user']['user_email'] + project_name + project_category + "doughnat-graph.svg"
    svg_path = 'static/graph/' + svg_name
    if 'to_delete' not in session.keys():
        session['to_delete'] = []
    session['to_delete'].append(svg_path)
...
def get_flask_weasy_pdf():
    ...
    if 'date_end' in request.args.keys():
        ...
        date_end = datetime.datetime.strptime(date_end, "%Y-%m-%d")
        date_range = date_start.strftime("%B %d, %Y")+" - "+date_end.strftime("%B %d, %Y")
    ...
    time_series_data = getTimeSeriesData(project_name, project_category, None, date_start, date_end)
    bar_pie_data = getBarPieData(project_name, project_category, None, date_start, date_end)
    selected_mentions = get_project_data(project_name, project_category, 1, 1, PAGE_SIZE, None, date_start, date_end).get_json()
    ...
    rendered_template = render_template('report.html', project_name=project_name, base_url=session['user']['user_email'] + project_name + project_category, reviews_list=selected_mentions, date_range=date_range)
...
def analyze_text():
    ...
    for sentence in sentences:
        r = requests.get("http://127.0.0.1:5000/get_sentiment/", params={"text": sentence})
        if r.status_code == 200:
...
def unsubscribe(encoded_email):
    ...
    if 'invalid_password_unsubscribe' in session.keys():
        ...
        return render_template('unsubscribe_form.html', email=decoded_email, invalid_password_unsubscribe=True)
...
def count_updates(user_id, project_id):
    data = list(mongo.db.ProjectData.aggregate([
        {"$match": {
            "_id": user_id
        }},
...
def weekly_update():
    user_project_list = mongo.db.ProjectData.find({}, {"_id": 1,
                                                       "user_email": 1,
                                                       "subscribed": 1,
                                                       "projects._id": 1,
                                                       "projects.project_name": 1,
    ...
    for user in user_project_list:
        ...
        for project in user['projects']:
            ...
            if days >= 0: #Change here days to 7..
                logging.warning("Here we'll scrape data for every project satisfy days >= 7")
                if project_category == 'Product':

                    def scrap_amazon():
                        os.chdir('Amazon')
                        subprocess.check_output(['scrapy', 'crawl', 'amazon_reviews', '-a', 'url=' + str(url),
                                                 '-a', 'user_id=' + str(user_id),
                                                 '-a', 'project_id=' + str(project_id),
                                                 '-a', 'timestamp=' + str(timestamp.date())])
                        os.chdir('..')
                    ...
                    def scrap_rotten_tomatoes():
                        os.chdir('Rotten_Tomatoes')
                        subprocess.check_output(['scrapy', 'crawl', 'rt_reviews', '-a', 'url=' + str(url),
                                                 '-a', 'user_id=' + str(user_id),
                                                 '-a', 'project_id=' + str(project_id),
                                                 '-a', 'timestamp=' + str(timestamp.date())])
                        os.chdir('..')
                    ...
                    def scrap_google():
                        os.chdir('Google')
                        subprocess.check_output(['scrapy', 'crawl', 'google_results', '-a', 'query=' + str(url),
                                                 '-a', 'user_id=' + str(user_id),
                                                 '-a', 'project_id=' + str(project_id),
                                                 '-a', 'timestamp=' + str(timestamp.date())])
                        os.chdir('..')
                ...
                updated_count[project_name] = {"project_category": project_category,
                                               "updated_count": count_updates(user_id, project_id)}
                #Update timestamp of project here...
                mongo.db.ProjectData.update({"_id": user_id, "projects._id": project_id},
                                   {"$set": {"projects.$.timestamp": datetime.datetime.strptime(
        ...
        if subscribed is True and len(updated_count) > 0:
            with app.app_context():
                message = Message('Data Updated', sender=app.config.get('MAIL_USERNAME'),
                                  recipients=[user_email])
                html = '<h1>' + "New data collected!" + '</h1>'
                temp_html = ''
                for project_name in updated_count.keys():
                    temp_html += '<h5>Project Name: ' + project_name + " Project Category: " + \
                                 updated_count[project_name]['project_category'] \
                                 + " New Data: " + str(updated_count[project_name]['updated_count']) + "</h5>"
                html += temp_html
                #unsubscribe_a = '<a href=' + "http://127.0.0.1/unsubscribe" + '>' + "Unsubscribe" + '</a>'
                current_app.config['SERVER_NAME'] = '127.0.0.1:5000'
                with current_app.test_request_context():
                    unsubscribe_a = '<a href=' +url_for('unsubscribe',
                                                    encoded_email=
                                                    base64.b64encode(user_email.encode(encoding='utf-8')),
                                                    _external=True) + '>' + "Unsubscribe" + '</a>'
                html += unsubscribe_a
                message.html = html
                try:
                    mail.send(message)
...
'''
Three main changes to persist....
1- get_sentiment limiter.exempt
2- Wait use in selenium in Rotten Tomatoes Bot
3- Use of langdetect in all bots...
...
if __name__ == '__main__':
    ...
    scheduler.start()
    atexit.register(lambda: scheduler.shutdown())#it shutdowns the thread of schedular which is run independantly...
