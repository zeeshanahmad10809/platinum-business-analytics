...
class GoogleResultsSpider(scrapy.Spider):
    ...
    def __init__(self, query, user_id, project_id, timestamp, *args, **kwargs):
        ...
        if timestamp == "None":
            ...
            logging.log(
                logging.INFO, "Searching for all time google search results...."
            )
            for url in search(query):
                urls.append(url)
        else:
            ...
            logging.log(
                logging.INFO, "Searching for last week google search results only...."
            )
        ...
        try:
            ...
            if self.last_update_date is not None:
                ...
                if created_date.date() <= self.last_update_date:
                    logging.log(
                        logging.INFO,
                        "Skiping scraping google search url because of outdated...",
                    )
                    return
            ...
            results = (
                ...
                + response.xpath(
                    "//h1[contains(translate(text(), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), '"
                    + self.query
                    + "')]"
                )
            )
    ...
    def extractDateCreated(self, response):
        ...
        try:
            ...
            if len(results) > 0:
                ...
                match_date = re.match(
                    "(\d{4}|\d{2})-(\d{2}|\d{1})-(\d{2}|\d{1})", extracted_date
                )
    ...
    def extractCategory(self, response):
        def get_translation(keyword):
            translation_table = keyword.maketrans(
        ...
        if (
            response.xpath("//meta[@name='keywords']/@content") is not None
            and len(response.xpath("//meta[@name='keywords']/@content")) > 0
        ):
            web_page_keywords = response.xpath("//meta[@name='keywords']/@content")[
                0
            ].extract()
        ...
        if (
            response.xpath("//meta[@name='description']/@content") is not None
            and len(response.xpath("//meta[@name='description']/@content")) > 0
        ):
